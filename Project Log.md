...
I have noticed that most of the frequencies begin around 120Hz. I am unsure if this is some kind of physical limitation of signal transfer, or some other impractical reception (such as distance or hardware required- cost, speed, etc). For example, it may not be practical to develop a radio at 30 Hz instead of 120 hz if it only travels 1 inch (using a theoretical "regressive analysis"). Perhaps the frequency is orthogonal to transmit power, and not exactly relevant.
